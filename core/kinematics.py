...
Rotation matrix : axis transformation
"""
...
class Kinematics:
    ...
    def rmxeuler(rot_mat):
        ...
        input
        rot_mat : 3x3 radian angle matrix
        output
        euler_angle  : scalar radian angle
        euler_vector : 3x1 numpy array
        """
        ...
        return euler_angle, euler_vector.T
    ...
    def quat_matrix(q):
        """
        q : quaternion [q_real, q_vec]^T (4x1 or 1x4)
    ...
    def quat_inv(q):
        """
        q : quaternion [q_real, q_vec]^T (4x1)
        Return : q_conj = [q_real, -q_vec]^T (4x1)
        """
    ...
    def quat_prod(quat1, quat2):
        ...
        quat1 : np.array (4x1)
            First quaternion [q_real, qx, qy, qz]^T where Q = q_real + qx i + qy j + qz k.
        quat2 : np.array (4x1)
            Second quaternion [p_real, px, py, pz]^T where P = p_real + px i + py j + pz k.
    ...
    def quat_unity(q):
        """
        q : np.array (4x1)
            Input quaternion [qw, qx, qy, qz]^T
        Returns:
            np.array
            Normalized quaternion [qw, qx, qy, qz]^T, such that the square of its norm equals 1.
        """
