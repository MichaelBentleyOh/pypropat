...
class Orbit:
	...
	def dayf_to_time(dayf):
		"""
		Time conversion Elapsed time in seconds to hours, minutes, seconds
		:param dayf: float type (Elapsed time in seconds)
		:return: 3 numpy array (hours, minutes, seconds)
		"""
	...
	def delkep(kep_el):
		...
		:return: 6 numpy array
						...
						(1) - 0
						(2) - 0
						(3) - variation rate of the right ascension of ascending node, in radians/second.
						(4) - variation rate of the argument of perigee, in radians/second.
						(5) - variation rate of the mean anomaly, in radians/second.
		"""
	...
	def djm(day, month, year):
		...
		:param day: float type
		:param month: float type
		:param year: float type
		:return: float type (Modified Julian Date)
		"""
	...
	def djm_inv(mjd):
		"""
		furnish the day, month, and year at zero hours of the day to the Modified Julian Date with reference
		:param mjd: float type (Modified Julian Date in days, refereed to 1950.0)
		:return: 3 numpy array (day, month, year)
		"""
	...
	def earth_shadow(sat_pos, sun_pos):
		"""
		Verify if a given position is or isn't in the earth shadow
		:param sat_pos: 3 numpy array (spacecraft coordinates in the inertial system, in meters)
		:param sun_pos: 3 numpy array (coordinates of the sun in the inertial frame, in meters)
		:return: boolean type (0: spacecraft is in the earth shadow / 1: spacecraft is illuminated by the sun)
		"""
	...
	def egm_acc(x):
		"""
		Compute the acceleration due to Earth in geocentric terrestrial coordinates.
		The acceleration is computed by lagrange expansion of the Earth's gravity potential.
		:param x: 3 numpy array (Position vector, in meters, in geocentric terrestrial coordinates)
		:return: 3 numpy array (Acceleration vector, in m/s^2, in geocentrical coordinates)
		"""
		r = np.linalg.norm(x)
		q = Orbit.egm_ae/r
		t = x[2]/r
		...
		Orbit.egm_pn[0] = 1.0
		Orbit.egm_pn[1] = 1.73205080756887730*u
		Orbit.egm_qn[0] = 1.0
		Orbit.egm_qn[1] = q

		for m in range(2, Orbit.egm_nmax + 1):
			Orbit.egm_pn[m] = u * np.sqrt(1.0 + 0.50 / m) * Orbit.egm_pn[m - 1]
			Orbit.egm_qn[m] = q * Orbit.egm_qn[m - 1]
		...
		# Outer n loop
		for m in range(0, Orbit.egm_nmax + 1):
			# Init
			pnm = Orbit.egm_pn[m]  # m=n sectoral
			dpnm = -m * pnm * tf
			pnm1m = pnm
			pnm2m = 0.0

			# Init Horner's scheme
			qc = Orbit.egm_qn[m] * Orbit.egm_cc[int(Orbit.egm_ip[m] + m)]
			qs = Orbit.egm_qn[m] * Orbit.egm_sc[int(Orbit.egm_ip[m] + m)]
			#print(Orbit.egm_ip[m])
			#print(m)
			#print(round(Orbit.egm_ip[m]+m))
			#print(Orbit.egm_ip)
			xc = qc * pnm
			xs = qs * pnm
			xcf = qc * dpnm
			...
			# Inner m loop
			for n in range(m + 1, Orbit.egm_nmax + 1):
				nn = n
				...
				# Inner sum
				if nn >= 2:
					qc = Orbit.egm_qn[n] * Orbit.egm_cc[int(Orbit.egm_ip[n] + m)]
					qs = Orbit.egm_qn[n] * Orbit.egm_sc[int(Orbit.egm_ip[n] + m)]
					xc = (xc + qc * pnm)
					xs = (xs + qs * pnm)
					xcf = (xcf + qc * dpnm)
					xsf = (xsf + qs * dpnm)
		...
		# Gradient
		vl = -gmr * Orbit.egm_conv_f * vl
		vf = gmr * Orbit.egm_conv_f * vf
		vr = -(gmr / r) * (1.0 + Orbit.egm_conv_f * vr)
	...
	def egm_difeq(t, x, mjd, dsec, ext_acc):
		"""
		Return with the time derivative of the satellite orbit position and velocity
		:param t: float type (time in seconds)
		:param x: 6 numpy array (Position and Velocity)
		:param mjd: float type (Modified Julian date referred to 1950)
		:param dsec: float type (Day time UTC in seconds of initial orbit ephemeris)
		:param ext_acc: 3 numpy array (External specific forces in m/s^2)
		:return: 6 numpy array (Time derviative of the state vector)
		"""
		xip = x[3:6]
		gwst = Orbit.gst(mjd, dsec + t)
		se = Orbit.inertial_to_terrestrial(gwst, x)
		xe = se[0:3]
		ae = np.concatenate([Orbit.egm_acc(xe),[0,0,0]],0)
		ai = Orbit.terrestrial_to_inertial(gwst,np.transpose(ae))
	...
	def egm_read_data(egm_data_file, nmax=0):
		"""
		Read data file and store in global variables
		:param egm_data_file: .dat file
		:param nmax: int type
		:return: X
		"""
		...
		Orbit.egm_order = int(num_data[0])
		Orbit.egm_length = copy.deepcopy(num_data[1])
		Orbit.egm_length = (Orbit.egm_order+2)*(Orbit.egm_order+1)/2-3
		Orbit.egm_conv_f = copy.deepcopy(num_data[2])

		Orbit.egm_cc = np.concatenate([[0, 0, 0], cf_data[2]],0)
		Orbit.egm_sc = np.concatenate([[0, 0, 0], cf_data[3]],0)

		Orbit.egm_ae = 6378136.3
		Orbit.egm_gm = 3986004.415e8
		Orbit.egm_pn = np.zeros(Orbit.egm_order+1)
		Orbit.egm_qn = np.zeros(Orbit.egm_order+1)
		Orbit.egm_ip = np.zeros(Orbit.egm_order+1)
		Orbit.egm_ip[0] = 0

		for n in range(1,Orbit.egm_order+1):
			Orbit.egm_ip[n] = Orbit.egm_ip[n-1] + n

		if nmax != 0:
			if nmax < Orbit.egm_order:
				Orbit.egm_nmax = nmax
			else:
				Orbit.egm_nmax = copy.deepcopy(Orbit.egm_order)
		else:
			Orbit.egm_nmax = copy.deepcopy(Orbit.egm_order)
		return
	...
	def geocentric_to_sph_geodetic(geoc):
		"""
		Transform rectangular terrestrial geocentric coordinates in spherical geodetic coordinates
		:param geoc: 3 numpy array (geocentric position x, y, z in meters)
		:return: 3 numpy array (geodetic coordinates vector east longitude (rad), geodetic latitude (rad), geodetic altitude (meters))
		"""
	...
	def gst(diju, time):
		"""
		Time conversion to greenwich aparent mean sidereal time refereed to the J2000.0
		:param diju: float type (Modified Julian Date in days, referred to 1950.0)
		:param time: float type (UT1 time complement in seconds)
		:return: float type (Greenwich aparent sidereal time in radians)
		"""
	...
	def igrf_field(date, alt, colat, elong):
		"""
		Synthesize geomagnetic field values using the 10th generation IGRF model (valid from 1900.0 to 2020.0).
		:param date: float type (decimal year, must be between 1900.0 and 2020.0 (inclusive))
		:param alt: float type (distance from the Earth's center in kilometers (> 3485 km))
		:param colat: float type (colatitude in radians (0 to 2pi))
		:param elong: float type (east longitude in radians (0 to 2pi))
		:return: 3 numpy array (geomagnetic field vector in nanotesla: [north, east, vertical])
		"""
	...
	def inertial_to_terrestrial(tesig, xi):
		"""
		Transforms geocentric inertial state vector into geocentric terrestrial coordinates (referred to Greenwich)
		:param tesig: float type (Greenwich sidereal time in radians)
		:param xi: 6 numpy array (geocentric inertial Position and Velocity)
		:return: 6 numpy array (geocentric terrestrial Position and Velocity)
		"""
	...
	def kepel_statvec(kepel):
		...
		:param kepel: 6 numpy array (Keplerian elements)
						(0) - semimajor axis of the orbit in meters.
						(1) - eccentricity.
						(2) - inclination in radians.
						(3) - right ascension of ascending node in radians.
						(4) - argument of perigee in radians.
						(5) - mean anomaly in radians.
		:return: 6 numpy array (state vector in meters and meters/second)
		"""
		EARTH_GRAVITY = 3.9860064e14  # Earth's gravitational constant [m3/s2]
	...
	def kepler(mean_anomaly, eccentricity):
		...
		:param mean_anomaly: float type (Mean anomaly in radians)
		:param eccentricity: float type (Eccentricity)
		:return: float type (Eccentric anomaly in radians)
		"""
	...
	def orbital_to_inertial_matrix(kepel):
		...
		:param kepel: 6 numpy array (Keplerian elements)
						...
						(4) - argument of perigee in radians.
						(5) - mean anomaly in radians.
		:return: 3x3 numpy array (rotation matrix form orbital to inertial frame)
		"""
		exc = kepel[1]
	...
	def proximus(angleinp, angleprox):
		"""
		Returns with angleinp in the nearest position from angleprox
		:param angleinp: float type (Input angle in radians)
		:param angleprox: float type (Given angle to compute variable in radians)
		:return: float type (Equals to anlgeinp, close to angleprox, in radians)
		"""
		test = 2*np.pi
	...
	def sph_geodetic_to_geocentric(spgd):
		"""
		Transform spherical geodetic coordinates (longitude, latitude and altitude)
		in rectangular terrestrial geocentric coordinates
		:param spgd: 3 numpy array (Geodetic coordinates)
						(0) - east longitude (rad)
						(1) - geodetic latitude (rad)
						(2) - geodetic altitude (meters)
		:return: 3 numpy array (Geocentric rectangular coordinates in meters, [x, y, z])
		"""
		EARTH_FLATNESS = 0.0033528131778969144
		EARTH_RADIUS	= 6378139.
	...
	def statvec_kepel(statv):
		"""
		Transforms the state vector into the corresponding keplerian elements in the same reference system
		:param statv: 6 numpy array (State vector in meters and meters/second)
		:return: 6 numpy array (Keplerian elements)
						(0) - semimajor axis of the orbit in meters.
						(1) - eccentricity.
						(2) - inclination in radians.
						(3) - right ascension of ascending node in radians.
	...
	def sun(djm, ts):
		...
		:param djm: float type (Modified julian date in days, referred to 1950.0)
		:param ts: float type (Fraction of the day in seconds)
		:return: 6 numpy array (Position vector of the sum)
	...
	def sun_dir(djm, ts):
		"""
		The subroutine sun calculates the direction vector (unit vector)
		of the Sun in the geocentric inertial system
		:param djm: float type (Modified julian date in days, referred to 1950.0)
		:param ts: float type (Fraction of the day in seconds)
		:return: 3 numpy array (Direction vector of the sun, [x, y, z])
		"""
		idays = djm - 18261
		tttt = idays + ts/86400
	...
	def sunsync_inc(sma, exc):
		"""
		Calculates the sunsynchronous inclination or a given orbit
		:param sma: float type (Orbit semi-major axis in meters)
		:param exc: float type (Orbit eccentricity)
		:return: float type (Sun-Synchronous inclination of the orbit in radians)
		"""
	...
	def sunsync_raan(eq_cross_time, gst0):
		"""
		Obtains the orbital right ascension of the ascending node,
		given the ascending equator crossing hour and the Greenwich sidereal time
		:param eq_cross_time: float type (Equator crossing hour of the ascending node, in radians)
		:param gst0: float type (Greenwich sidereal time at 0:00:00 h UTC)
		:return: float type (Right ascension of the ascending node, in radians)
		"""
	...
	def sunsync_sma(exc, inc, q):
		"""
		Obtains the orbital semimajor axis of a low Earth orbit,
		given the oribtal recovering factor q
		:param exc: float type (Orbital eccentricity)
		:param inc: float type (Orbital inclination in radians)
		:param q: float type (Sun-synchronous orbit recovering factor)
		:return: float type (Recurrent orbital semimajor axis, in meters)
		"""
		earth_gravity = 3.9860064e14
	...
	def terrestrial_to_inertial(tesig, xt):
		"""
		Transforms geocentric terrestrial coordinates
		referred to Greenwich into geocentric inertial coordinates
		:param tesig: float type (Greenwich sidereal time in radians)
		:param xt: 6 numpy array (Geocentric terrestrial position and velocity)
		:return: 6 numpy array (Geocentric inertial position and velocity)
		"""
	...
	def time_to_dayf(hours, minutes, seconds):
		...
		:param hours: int type (Day hour)
		:param minutes: int type (Minutes)
		:param seconds: float type (Seconds and fraction of second)
		:return: float type (Day elapsed time in seconds)
		"""
	...
	def visviva(a,r):
		"""
		find the solution of visviva equation
		:param a: float type (Semi-major axis in meters)
		:param r: float type (Radius in meters)
		:return: float type (Velocity in meters/second)
		"""
		mu = 3.986e14
