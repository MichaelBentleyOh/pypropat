import numpy as np
# from pypropat.core.kinematic import Kinematics
import kinematics
# from pypropat.core.orbit import Orbit
import orbit
...
# kin = Kinematics()
# skew = kin.cross_matrix(np.array[2,3,1])
...
tstep = 50
tend = 6000
time = np.arange(0, tend + tstep, tstep)
...
body_angular_rate = np.zeros((3,n))
...
for t in np.arange(tstart, tend + tstep, tstep):
    ...
    r = np.array([sv_nm[0], sv_nm[1], sv_nm[2]])
    v = np.array([vi_nm[0], vi_nm[1], vi_nm[2]])
    ...
    LVLH_rot = np.concatenate([x_LVLH, y_LVLH, z_LVLH],0).T
    LVLH_rot = LVLH_rot.reshape(3,3)
    # print(LVLH_rot)
    LVLH_quat = kinematics.rmxquat(LVLH_rot)
    desired_state = np.concatenate([LVLH_quat,np.zeros(3)])
    ...
    sol = solve_ivp(eom, (t, t + tstep), initial_state, rtol=1e-12, atol=1e-12)
    # print(sol)
    # print("Shape of sol.y:", sol.y.shape)
    # print("Shape of body_quat[ic, :]:", body_quat[ic, :].shape)

    initial_state = sol.y[:, -1]

    body_quat[ic, :] = sol.y[0:4, -1]
    body_angular_rate[:, ic] = sol.y[4:7, -1]
...
body_X_hlr = ax.quiver(0, 0, 0, 1, 0, 0, color='r', linewidth=2, label='body X-axis')
body_Y_hlr = ax.quiver(0, 0, 0, 0, 1, 0, color='g', linewidth=2, label='body Y-axis')
body_Z_hlr = ax.quiver(0, 0, 0, 0, 0, 1, color='b', linewidth=2, label='body Z-axis')
...
def update_quivers_LVLH(num):
    ...
    print(body_quat[num,:])
    quat = body_quat[num,:].T
    rot = kinematics.quatrmx(quat)
    ...
    body_X = rot @ np.array([axis_length,0,0])
    body_Y = rot @ np.array([0,axis_length,0])
    body_Z = rot @ np.array([0,0,axis_length])
    ...
    body_X_hlr.set_segments([[(r[0], r[1], r[2]),
                              (r[0] + body_X[0], r[1] + body_X[1], r[2] + body_X[2])]])
    body_Y_hlr.set_segments([[(r[0], r[1], r[2]),
                              (r[0] + body_Y[0], r[1] + body_Y[1], r[2] + body_Y[2])]])
    body_Z_hlr.set_segments([[(r[0], r[1], r[2]),
                              (r[0] + body_Z[0], r[1] + body_Z[1], r[2] + body_Z[2])]])
...
# anim.save('orbit_propagation.gif', writer=PillowWriter(fps=10))
