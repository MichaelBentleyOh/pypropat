...
quat = Kinematics.ezxzquat(eulzxz)
...
for t in np.arange(tstart, tend+tstep, tstep):
	...
	if flag_mag == 0:
		def func(t, x, ext_torque=ext_torq, tensin=iner, teninv=invin):
			return Control.rigidbody_dynamics(t, x, ext_torque, tensin, teninv)
		...
		geoc = Orbit.inertial_to_terrestrial(Orbit.gst(mjd, dfra + t), stat.squeeze(0))
		# Earth's magnetic field
		sphe = Kinematics.rectangular_to_spherical(geoc)
		alt = sphe[2] / 1000
		elong = sphe[0]
		colat = np.pi/2 - sphe[1]
		earth_field = 1E-9*Orbit.igrf_field(year_frac, alt, colat, elong)
		def func(t, x, ext_torq, iner, invin, mag_mom, earth_field):
			return Control.rigidbody_dynamics(t, x, ext_torq, iner, invin,	mag_mom, earth_field)
	...
	eulzxz = Kinematics.quatezxz(quat)		# euler angles
