...
RE = 6731				# km
alt_init = float(input('Altitude of an initial orbit [km]: '))			# altitude km
...
# Final Orbit
alt_final = float(input('Altitude of a final orbit [km]: '))			# km
...
lb_nu_trans = np.arccos(2/ratio_r-1)

nu_trans = float(input('True anomaly change during transfer [deg]: '))*deg2rad
while nu_trans < lb_nu_trans or nu_trans > np.pi:
    lb_nu_trans_deg = np.ceil(lb_nu_trans/deg2rad)
    print('Error! You should choose in [' + '%d' %(lb_nu_trans_deg) + ' 180]')
    nu_trans = float(input('True anomaly change during transfer [deg]: ')) * deg2rad

e_trans = (1 - ratio_r)/(ratio_r*np.cos(nu_trans) - 1)

mean_trans = np.arctan2(-np.sqrt(1-e_trans**2)*np.sin(nu_trans), -e_trans-np.cos(nu_trans)) + \
    np.pi - e_trans*(np.sqrt(1-e_trans**2)*np.sin(nu_trans))/(1+e_trans*np.cos(nu_trans))
...
# Make Initial Orbit
tstart_init = 0
tend_init = int(np.fix(2*np.pi*np.sqrt(r_init**3/mu)))
tstep_init = tend_init/100
n_init = int(np.fix(tend_init/tstep_init))+1


r_rx_init   = np.zeros(n_init)
r_ry_init   = np.zeros(n_init)
r_rz_init   = np.zeros(n_init)

ic_init = 0
print('\n')
for t in tqdm(np.arange(tstart_init, tstart_init+tend_init+tstep_init, tstep_init), desc='Loading an Initial Orbit...  '):
    sol_init = solve_ivp(func, (t, t + tstep_init), tuple(stat_init.squeeze(0)), rtol=1e-12, atol=1e-12)
    Y = sol_init.y

    sv_nm = Y[:, -1]  # propagated state vector

    r_rx_init[ic_init] = sv_nm[0]
    r_ry_init[ic_init] = sv_nm[1]
    r_rz_init[ic_init] = sv_nm[2]
...
# Make Final Orbit
tstart_final = 0
tend_final = int(np.fix(2*np.pi*np.sqrt(r_final**3/mu)))
tstep_final = tend_final/100
n_final = int(np.fix(tend_final/tstep_final))+1

r_rx_final   = np.zeros(n_final)
r_ry_final   = np.zeros(n_final)
r_rz_final   = np.zeros(n_final)

ic_final = 0
for t in tqdm(np.arange(tstart_final, tstart_final+tend_final+tstep_final, tstep_final), desc='Loading a final Orbit...     '):
    sol_final = solve_ivp(func, (t, t + tstep_final), tuple(stat_final.squeeze(0)), rtol=1e-12, atol=1e-12)
    Y = sol_final.y

    sv_nm = Y[:, -1]  # propagated state vector

    r_rx_final[ic_final] = sv_nm[0]
    r_ry_final[ic_final] = sv_nm[1]
    r_rz_final[ic_final] = sv_nm[2]
    stat_final = sv_nm.reshape((1, 6))  # state vector update

    ic_final = ic_final + 1

tstart = 0
tend1 = int(np.fix(mean_trans/np.sqrt(mu/a_trans**3)))
tstep1 = tend1/100
tend2 = tend_final
tstep2 = tend2/100

n = int(np.fix((tend1-tstart)/tstep1))+1
n2 = n + int(np.fix(tend2/tstep2)) + 1
...
ic = 0
for t in tqdm(np.arange(tstart, tstart+tend1+tstep1, tstep1), desc='Loading a Transfer Orbit...  '):
    sol = solve_ivp(func, (t, t + tstep1), tuple(stat.squeeze(0)), rtol=1e-12, atol=1e-12)
    Y = sol.y
...
fpa_trans = np.arctan2(e_trans*np.sin(nu_trans), (1+e_trans*np.cos(nu_trans)))

v_final = np.sqrt(mu/r_final)
v_trans_f = np.sqrt(2*mu/r_final - mu/a_trans)
delv2 = np.sqrt(v_trans_f**2 + v_final**2 - 2*v_trans_f*v_final*np.cos(fpa_trans))

angm_vec = np.cross(stat_final[0,:3], stat_final[0,3:])

direc_vec = np.cross(angm_vec, stat_final[0,:3])

stat_final[0, 3:] = np.linalg.norm(v_final) * direc_vec / np.linalg.norm(direc_vec)


for t in tqdm(np.arange(tend1, tend1+tend2+tstep2, tstep2), desc='Loading After Transfer...    '):
    sol = solve_ivp(func, (t, t + tstep2), tuple(stat_final.squeeze(0)), rtol=1e-12, atol=1e-12)
    Y = sol.y
...
ax.plot3D(r_rx_final/1000, r_ry_final/1000, r_rz_final/1000, 'r', linewidth=1)  # final orbit
...
# Axis settings
lim_len = r_final/1000*1.2
ax.set_xlim([-lim_len, lim_len])
ax.set_ylim([-lim_len, lim_len])
ax.set_zlim([-lim_len, lim_len])
ax.grid(True)
...
def update_quivers(num):
    ...
    targ_X = R @ ref_X*1000*5
    targ_Y = R @ ref_Y*1000*5
    targ_Z = R @ ref_Z*1000*5
    ...
    ax.set_title('Propagation Simulation Time : {:.4f}hour'.format(r_time[num]/3600))
